...
[![Tests](https://img.shields.io/badge/tests-200%20passing-brightgreen.svg)](https://github.com/yourusername/FedTracker-Pro)
...
fingerprint: { enabled: true, ... }    # 参数指纹（逐客户端唯一）
adaptive_allocation: { enabled: true, ... }  # 自适应分配
crypto:      { enabled: true, ... }    # 密码学验证（含 client_id 绑定）
...
verification:
  ...
  cross_verify_client_id: true    # 交叉验证签名与指纹的 client_id
```
...
**目的**：在训练完成后，对可疑模型执行三层所有权验证并溯源到具体泄露客户端。
...
print(f"所有权: {is_owner}, 泄露客户端: {client_id}, 置信度: {confidence:.4f}")
# 三层验证逻辑:
#   Level 1 — 指纹相似度 >= level1_threshold (0.75)，自动匹配最相似客户端
#   Level 2 — 密码学签名验证通过（含 client_id 交叉验证）
...
# 查看所有客户端的指纹相似度（调试用）
if framework.fingerprint_registry is not None:
    sims = framework.fingerprint_registry.get_all_similarities(suspicious_model)
    for cid, score in sorted(sims.items()):
...
│   │   ├── protected_client.py      # 受保护客户端（训练后自动嵌入指纹）
│   │   └── fed_tracker_pro.py       # 主框架类（含逐客户端溯源）
│   ├── datasets/
...
| 阶段 9 | 工程化（`requirements-dev` / 脚本 / 文档 / 集成测试 / diffusion 模型） | ✅ 完成 | — |
| 阶段 10 | 逐客户端溯源（`ClientFingerprintRegistry` + `ProtectedClient`） | ✅ 完成 | 200 total |
| 阶段 11 | 端到端实验与论文结果复现 | ⏳ 待运行 | — |

**总测试**: 200 passing（197 unit + 3 integration）
...
│   ├── 逐客户端唯一指纹 (ClientFingerprintRegistry, seed = base + client_id)
│   └── 遗忘增强定位 (Unlearning-Guided)
├── 验证检测层
│   ├── Level 1 — 统计验证 (FSS 指纹相似度 ≥ 0.75，自动匹配泄露客户端)
│   ├── Level 2 — 密码学验证 (ECDSA 数字签名 + client_id 交叉验证)
│   └── Level 3 — 行为验证 (CL 水印准确率 ≥ 0.9)
└── 溯源追踪层
    ├── 逐客户端指纹匹配 (identify_client 遍历余弦相似度)
    ├── 密码学 client_id 绑定 (签名中编码客户端身份)
...
# 基线实验
make run-baseline                     # python -m experiments.exp_baseline
make run-ablation                     # python -m experiments.exp_ablation
